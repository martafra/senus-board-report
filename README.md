...
React (Vite) SPA at `apps/web`: login (JWT stored in `localStorage`), a dashboard with one page per
Board Report section, each showing an `InsightPanel` (the AI commentary, with a Regenerate button
and a "Generated by \<model\> on \<date\>" caption for transparency), a chart, and a table
(`MetricsTable`) with a Reported/Modelled badge per period and a hover/focus-accessible info
tooltip per metric (shadcn/ui `Tooltip`, so it works with a mouse, keyboard or touch). Data
fetching goes through TanStack Query calling the API above.
